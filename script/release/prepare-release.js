...
if (!bumpType && !args.notesOnly) {
  console.log('Usage: prepare-release [stable | minor | beta | nightly]' + ' (--stable) (--notesOnly) (--automaticRelease) (--branch)');
  process.exit(1);
}
...
async function pushRelease(branch) {
  ...
  if (pushDetails.exitCode === 0) {
    console.log(`${pass} Successfully pushed the release.  Wait for ` + 'release builds to finish before running "npm run release".');
  } else {
...
prepareRelease(!args.stable, args.notesOnly).catch((err) => {
  console.error(err);
  process.exit(1);
});
