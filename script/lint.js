  ...
  },
  {
    ...
    run: async (opts, filenames) => {
      ...
      if (badResults.length) {
        console.log(formatter.format(badResults));
      }
      ...
      if (opts.fix) {
        ...
        let isOK = true;
        for (const filename of filenames) {
          const isFileOK = prettier.check(await fs.readFile(filename, 'utf8'), prettierConfig);
          isOK = isOK && isFileOK;
          if (!isFileOK) {
            console.error(`${filename}: Prettier check failed...`);
          }
        }
        if (!isOK) {
          console.error('Code style issues found in the above file(s).  Forgot to run Prettier?  Run "yarn lint:js --fix" to auto-fix');
          process.exit(1);
        }
      }
    },
