...
void ChromeNSSCryptoModuleDelegate::ShowDialog(const std::string& slot_name,
  ...
  electron::api::App::Get()->ShowCryptoModulePasswordDialog(
      slot_name, retry, server_.host(),
      base::BindRepeating(&ChromeNSSCryptoModuleDelegate::GotPassword,
...
CreateCryptoModuleBlockingPasswordDelegate(const net::HostPortPair& server) {
  return new ChromeNSSCryptoModuleDelegate(server);
}
